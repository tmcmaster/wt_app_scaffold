...
class AppScaffoldProviderMonitor
  ...
  void didDisposeProvider(ProviderBase provider, ProviderContainer containers) {
    log.d(
      'ProviderDebugger : didDisposeProvider : ${provider.name ?? provider.runtimeType}',
    );
    if (provider.name != null) {
      if (provider.name!.endsWith('family')) {
        families[provider.name!] = (families[provider.name!] ?? 0) - 1;
        ...
        if (values.containsKey(provider.name)) {
          ...
          log.w(
            'Provider was not in the provider list when it was disposed: ${provider.name}',
          );
        }
      }
    } else {
